...
export function InsightCard({ insight }: { insight: ContentInsight }) {
  // Gracefully handle either the old {title, body} shape or the new ContentInsight shape
  ...
  return (
    <article className="group flex min-h-[178px] flex-col justify-between rounded-xl border border-zinc-200 bg-gradient-to-br from-white to-[#f8faf9] p-3.5 shadow-[0_1px_3px_rgba(15,23,42,0.02)] transition-all hover:-translate-y-0.5 hover:border-[#cbd8d3] hover:shadow-[0_10px_24px_-16px_rgba(15,23,42,0.35)]">
      <div>
        <div className="flex items-start gap-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-zinc-200 bg-white shadow-sm">
            <FileText size={14} className="text-[#17404b]" />
          </div>
          <div className="flex-1">
            <h4 className="line-clamp-2 text-[12.8px] font-bold leading-[1.35] text-zinc-950">{title}</h4>
            {theme && (
              <span className="mt-2 inline-flex items-center rounded-full border border-zinc-200 bg-white px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-[0.16em] text-zinc-500">
                {theme}
              </span>
            )}
          </div>
