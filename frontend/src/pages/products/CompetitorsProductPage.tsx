      ...
      <div className="w-full">
        <DashboardCard className="w-full">
          <div className="flex items-center justify-between gap-4 px-5 py-4">
            <div>
              ...
              <p className="mt-0.5 text-[12px] font-medium text-zinc-400">
                Manage brands tracked across AI answers for PromptPulse
              </p>
            </div>
          ...
          <div className="space-y-2 px-4 pb-4 md:hidden">
            {competitors.map((c) => (
              <div key={c.name} className="rounded-2xl border border-zinc-100 bg-white p-3 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    <Favicon domain={c.domain} />
                    <div className="min-w-0">
                      <p className="truncate text-[13px] font-black text-zinc-950">{c.name}</p>
                      <p className="truncate text-[11px] font-medium text-zinc-400">https://{c.domain}</p>
                    </div>
                  </div>
                  <span className="inline-flex shrink-0 items-center rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-black text-emerald-700">
                    {c.status}
                  </span>
                </div>
                <div className="mt-3 grid grid-cols-2 gap-2 text-[11.5px]">
                  <div className="rounded-xl bg-zinc-50 p-2">
                    <p className="font-black uppercase tracking-[0.12em] text-zinc-400">Visibility</p>
                    <p className="mt-1 font-black text-zinc-950">{c.visibility}</p>
                  </div>
                  <div className="rounded-xl bg-zinc-50 p-2">
                    <p className="font-black uppercase tracking-[0.12em] text-zinc-400">Sentiment</p>
                    <p className="mt-1 inline-flex items-center gap-1.5 font-bold text-zinc-700">
                      <SentimentDot score={c.sentiment} /> {c.sentiment}
                    </p>
                  </div>
                  <div className="rounded-xl bg-zinc-50 p-2">
                    <p className="font-black uppercase tracking-[0.12em] text-zinc-400">Position</p>
                    <p className="mt-1 font-bold text-zinc-700"># {c.position}</p>
                  </div>
                  <div className="rounded-xl bg-zinc-50 p-2">
                    <p className="font-black uppercase tracking-[0.12em] text-zinc-400">Mentions</p>
                    <p className="mt-1 inline-flex items-center gap-2 font-bold text-zinc-700">
